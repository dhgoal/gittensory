...
const BAND_ORDER: Array<keyof MaintainerQueueHealth["bandCounts"]> = ["low", "medium", "high", "critical"];
...
export function QueueHealthCard({ queueHealth }: { queueHealth?: MaintainerQueueHealth }) {
  ...
  const { openPullRequests, stalePullRequests, draftPullRequests, unlinkedPullRequests } = queueHealth;
  const ageTotal =
    queueHealth.ageBuckets.under7Days + queueHealth.ageBuckets.days7To30 + queueHealth.ageBuckets.over30Days;
    ...
    >
      ...
      <div className="mt-4 space-y-1.5">
        ...
        <div className="flex flex-wrap gap-x-3 gap-y-1 font-mono text-token-2xs text-muted-foreground">
          {(Object.keys(AGE_LABEL) as Array<keyof MaintainerQueueHealth["ageBuckets"]>).map((bucket) => (
            <span key={bucket}>
